...
def test_real_hybrid_login():
    ...
    try:
        ...
        # window.localStorage.setItem('Auth_token', arguments[1]);
        # """, username, token)
        
        token_value = token
        username_value = username # Use whatever key holds that 'efee428-b4' string

        # Pass them to your cookies
        driver.add_cookie({"name": "tokenp_", "value": token_value})
        driver.add_cookie({"name": "user", "value": username_value})
